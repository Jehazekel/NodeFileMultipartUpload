...
const FILE_TYPE = 'THUMBNAIL';

// for uploading thumbnails
ThumbnailRouter.post("/:videoId", attachmentUpload, async (req, res, next) => {
  try {
    // request body params
...
ThumbnailRouter.post("/create_session/:videoId", multer().none(), async (req, res, next) => {
  try {
    ...
    res.send({ success: false, message: 'Failed to create thumbnail upload session ' });
  }
}
);

// recieve thumbnail multipart file 
ThumbnailRouter.post("/file_parts/:videoId", multer().single('filePart'), async (req, res, next) => {
  try {
    ...
    res.send({ success: false, message: 'Failed to save thumbnail file part' });
  }
}
);

// complete thumbnail multipart file 
ThumbnailRouter.post("/complete/:videoId", multer().none(), async (req, res, next) => {
  try {
    ...
    res.send({ success: false, message: 'Failed to complete thumbnail file parts upload' });
  }
}
);
